...
for fold in range(5):
    ...
    df_train1 = pd.read_csv(f'train_fold_{fold+1}.csv')
    df_val = pd.read_csv(f'test_fold_{fold+1}.csv')
    ...
    for epoch in range(epochs):
        ...
        if val_acc >= best_acc:
            ...
            torch.save(model.state_dict(), f"weight/best_model{fold+1}epoch.pth")
        if epoch == 499:
            train_acc_result.append(train_acc)
            val_acc_result.append(val_acc)
            Precision_result.append(Precision)
            Sensitivity_result.append(Sensitivity)
            SP_result.append(Specificity)
            F1_result.append(F1)
            MCC_result.append(MCC)
            auc_score_result.append(auc_score)
        print(
...
print(F1_result)
print(MCC_result)
print(auc_score_result)
# 计算平均值
avg_train_acc = np.mean(train_acc_result)
avg_val_acc = np.mean(val_acc_result)
avg_Precision = np.mean(Precision_result)
avg_Sensitivity=np.mean(Sensitivity_result)
avg_SP = np.mean(SP_result)
avg_F1 = np.mean(F1_result)
avg_MCC = np.mean(MCC_result)
avg_AUC = np.mean(auc_score_result)

# 输出最终的平均结果
print(f"Cross-validation Results (Average over 5 folds):")
print(
    f'avg_train_acc:{avg_train_acc}, avg_val_acc:{avg_val_acc}, avg_prec:{avg_Precision} avg_SE:{avg_Sensitivity}, avg_SP:{avg_SP} ,avg_f1:{avg_F1} ,avg_MCC:{avg_MCC}, avg_AUC:{avg_AUC}')