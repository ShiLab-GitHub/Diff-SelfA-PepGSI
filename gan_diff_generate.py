...
if __name__ == '__main__':
    ...
    if CUDA:
        ...
        oracle_samples = torch.Tensor(np.array(all_data)).type(torch.LongTensor)
    ...
    gen.load_state_dict(torch.load(r'weight/gen_new.pth', map_location=torch.device('cpu')))
    dis.load_state_dict(torch.load(r'weight/dis_new.pth', map_location=torch.device('cpu')))
    ...
    f = open('outputs_len6.txt', 'w+')
    f_seq = open("seq_len6.txt", 'w+')
    print('\nGeneration Start')
