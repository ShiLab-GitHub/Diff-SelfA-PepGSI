...
loss_d = []
MAX_SEQ_LEN = 18
data = pd.read_csv('data/data_all_3descriptor.csv', skiprows=1, usecols=range(3), header=None, names=['ID', 'seq', 'len'])
...
def train_generator_MLE(gen, gen_opt, oracle, real_data_samples, epochs):
    for param in gen.embeddings.parameters(): #冻结前面的embeddings层
        param.requires_grad = False
    for param in gen.diff.parameters():  # 冻结前面的特征提取层
        param.requires_grad = False
...
def train_discriminator(discriminator, dis_opt, real_data_samples, generator, oracle, d_steps, epochs):
    ...
    for param in discriminator.embeddings.parameters(): #冻结前面的embeddings层
        param.requires_grad = False
    for param in discriminator.gru.parameters():  # 冻结前面的gru层
        param.requires_grad = False
...
gen_new_model='weight/gen_new.pth'
dis_new_model='weight/dis_new.pth'
if __name__ == '__main__':
    ...
    # 加载预训练权重
    pretrained_gen_weights = torch.load("weight/gen_500.pth")
    pretrained_dis_weights = torch.load("weight/dis_500.pth")
    gen.load_state_dict(pretrained_gen_weights)
    dis.load_state_dict(pretrained_dis_weights)
    if CUDA:
        oracle = oracle.cuda()
        gen = gen.cuda()
        dis = dis.cuda()
    ...
    torch.save(gen.state_dict(), gen_new_model)
    torch.save(dis.state_dict(), dis_new_model)
    #保存模型参数